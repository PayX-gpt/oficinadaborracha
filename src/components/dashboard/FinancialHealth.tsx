      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            {score > 60 ? "Sua oficina está saudável." : score > 30 ? "Atenção: margem pode melhorar." : "Alerta: margem muito baixa."}
            {` Margem líquida de ${margem.toFixed(1)}%.`}
          </p>
        </div>

        <div className="space-y-3">
          <p className="text-[11px] uppercase tracking-[0.05em] text-muted-foreground font-medium">Fluxo de Caixa</p>
          ...
          <div className="flex flex-wrap gap-2 text-[11px]">
            <span className="text-emerald-500">+{formatCurrency(totalReceita)} entradas</span>
            <span className="text-red-500">-{formatCurrency(totalDespesas + totalTaxas)} saídas</span>
          </div>
        </div>

        <div className="space-y-3">
          <p className="text-[11px] uppercase tracking-[0.05em] text-muted-foreground font-medium">Resumo do Período</p>
          <p className="text-2xl font-bold text-foreground">{formatCurrency(totalReceita)}</p>
          <p className="text-xs text-muted-foreground">Lucro líquido: {formatCurrency(lucroLiquido)}</p>
          ...
          <p className="text-xs text-muted-foreground">Custo total: {formatCurrency(totalCusto)}</p>
        </div>
      </div>
    </motion.div>
  );
