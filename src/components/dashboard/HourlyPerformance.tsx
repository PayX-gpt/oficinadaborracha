    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.85, duration: 0.4 }}
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { icon: TrendingUp, label: "Mais Lucrativa", value: peakHour.hour, sub: formatCurrency(peakHour.value), color: "text-emerald-500" },
            { icon: Activity, label: "Mais Movimentada", value: busiestHour.hour, sub: `${busiestHour.services} serv.`, color: "text-blue-500" },
            { icon: Clock, label: "Total Serviços", value: String(totalServices), sub: "", color: "text-primary" },
            { icon: BarChart3, label: "Média/Hora", value: totalServices > 0 ? (totalServices / 12).toFixed(1) : "0", sub: "", color: "text-violet-500" },
          ].map((m) => (
