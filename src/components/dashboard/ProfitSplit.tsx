    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75, duration: 0.4 }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-1">
          {waterfall.map((w, i) => (
            <motion.div key={w.label} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.05 }}
              className={`flex items-center justify-between py-1.5 px-2 rounded text-xs ${w.type === "total" || w.type === "result" ? "bg-secondary/50" : ""}`}>
              <span className={`${w.type === "total" || w.type === "result" ? "text-foreground font-bold" : "text-muted-foreground"}`}>
                {w.type === "negative" ? "→ " : ""}{w.label}
              </span>
              <span className={`font-medium ${w.type === "negative" ? "text-red-400" : w.type === "result" ? "text-primary font-bold" : "text-emerald-500"}`}>
        ...
        <div className="space-y-3">
          {socios.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-4">Cadastre sócios nas configurações</p>
          ) : socios.map((s) => (
            <div key={s.nome} className="rounded-lg p-3 bg-secondary/20 space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-xs font-bold text-primary">{s.nome[0]}</div>
                  <div>
