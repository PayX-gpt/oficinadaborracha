...
import { Plus, Trash2, ChevronLeft, Car, User, Wrench, CreditCard, StickyNote, Save, Loader2, Building2 } from "lucide-react";
...
const LaunchManual = () => {
  ...
  const handleMetodoChange = (m: string) => { setMetodo(m); setTaxa(String(taxas[m] ?? 0)); };
  ...
  return (
    <div className="space-y-4 pb-44">
      ...
      <SectionCard icon={Wrench} title="Itens do Serviço">
        <AnimatePresence>
          {items.map((item, idx) => (
            <motion.div key={item.id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="space-y-2 rounded-lg bg-secondary/30 p-3 border border-border/30">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {paymentMethods.map((m) => (
                <button key={m} onClick={() => handleMetodoChange(m)} className={`rounded-lg px-2 py-2 text-[11px] font-medium transition-all border ${metodo === m ? "bg-primary/20 border-primary/40 text-primary" : "bg-secondary/30 border-border/30 text-muted-foreground hover:border-primary/20"}`}>
                  {m}
                </button>
              ))}
            </div>
      ...
      <div className="fixed bottom-16 md:bottom-0 left-0 right-0 z-40 md:left-64">
        <div className="mx-auto max-w-4xl px-3 pb-2 md:pb-4">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="rounded-xl p-3 border border-primary/20" style={{ background: "rgba(14,20,35,0.95)", backdropFilter: "blur(20px)", boxShadow: "0 -4px 24px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.03)" }}>
