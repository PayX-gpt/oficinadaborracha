...
import { Camera, Sparkles, ChevronLeft, ImageIcon, Loader2, CheckCircle, Save } from "lucide-react";
          ...
          {!aiResult ? (
            <div className="flex gap-2">
              ...
              <Button onClick={handleAnalyze} disabled={analyzing} className="flex-1 h-10 bg-primary text-primary-foreground hover:bg-primary/90 gap-2 text-sm" style={{ boxShadow: "0 0 16px rgba(245,158,11,0.2)" }}>
                {analyzing ? <><Loader2 className="h-4 w-4 animate-spin" /> Analisando...</> : <><Sparkles className="h-4 w-4" /> Analisar com IA</>}
              </Button>
            </div>
          ) : (
